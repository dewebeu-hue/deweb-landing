...
export function CjenikHrRequestForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result: { ok?: boolean; error?: string; fields?: CjenikHrOrderField[]; accepted?: boolean; publicOrderToken?: string } = await response.json().catch(() => ({}));
  ...
  return (
    <form onSubmit={submit} noValidate className="rounded-2xl border border-[#bfd4d8] bg-white p-5 shadow-[0_20px_55px_rgba(8,42,61,0.10)] sm:p-7">
      ...
      {step === 1 ? <>
        ...
        <p className="mt-5 rounded-lg bg-[#f5f9fa] px-4 py-3 text-sm leading-6 text-muted">{isStandardOrder ? `Cijena paketa je ${formatCjenikHrPrice(packagePrice)}. Nakon provjere podataka priprema se ponuda; ovo nije trenutna kupnja.` : "Drugi sustav ostaje ručni upit bez automatske ponude, naplate ili isporuke."}</p>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-teal-dark">{isStandardOrder ? "Podatci za ponudu" : "Podatci za upit"}</p><p className="mt-1 text-sm text-muted">Hrvatska je trenutačno jedina podržana država naplate.</p></div><button type="button" onClick={() => setStep(1)} className="min-h-11 text-sm font-black text-teal-dark underline underline-offset-4">Promijenite paket</button></div>
        ...
        <p className="mt-5 text-xs leading-5 text-muted">Ne šaljite lozinke, pristupne podatke, kartične podatke ni poslovne dokumente. Podatke koristimo za obradu upita ili pripremu ponude i povezane poslovne evidencije. <a className="font-bold text-teal-dark underline underline-offset-2" href="/privatnost">Politika privatnosti</a>.</p>
        ...
        <button type="submit" disabled={sending} className="mt-5 inline-flex min-h-[52px] w-full items-center justify-center rounded-lg bg-teal px-6 text-sm font-black text-white shadow-[0_12px_25px_rgba(0,77,88,0.17)] transition hover:bg-teal-dark disabled:cursor-wait disabled:opacity-70 focus-visible:outline focus-visible:outline-4 focus-visible:outline-orange-dark">{sending ? "Šaljemo…" : isStandardOrder ? "Zatražite ponudu" : "Pošaljite upit"}</button>
      </>}
    </form>
  );
}
