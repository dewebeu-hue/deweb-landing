...
const messages: Record<CjenikHrPackage, { title: string; copy: string }> = {
  plugin: { title: "Upit za plugin je zaprimljen.", copy: "Provjerit ćemo osnovnu kompatibilnost vašeg WordPressa i javiti se s informacijama o isporuci." },
  setup: { title: "Upit za postavljanje je zaprimljen.", copy: "Pregledat ćemo navedenu domenu i javiti se s potrebnim preduvjetima za instalaciju i početno podešavanje." },
    ...
    <>
      ...
      <main className="hero-surface grid min-h-[70vh] place-items-center px-[18px] py-16">
        <section className="w-full max-w-2xl rounded-3xl border border-[#bfd4d8] bg-white p-7 text-center shadow-[0_24px_65px_rgba(8,42,61,0.12)] sm:p-12" aria-labelledby="thanks-title">
          ...
          <p className="mt-4 text-sm leading-6 text-muted">Ako odgovor ne vidite u uobičajenom roku, provjerite mapu neželjene pošte ili nam pišite na <a className="font-black text-teal-dark underline underline-offset-4" href="mailto:dinko@deweb.hr">dinko@deweb.hr</a>.</p>
