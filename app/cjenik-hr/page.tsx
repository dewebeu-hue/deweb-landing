    ...
    <>
      <header className="sticky top-0 z-30 border-b border-line bg-white/95 backdrop-blur">
        <div className={`${frame} flex min-h-[72px] items-center justify-between gap-4`}>
          ...
          <a href="#upit" className="inline-flex min-h-11 items-center rounded-lg bg-teal px-4 text-sm font-black text-white hover:bg-teal-dark focus-visible:outline focus-visible:outline-4 focus-visible:outline-orange-dark">{isPreview ? "Testiraj kupnju" : "Zatražite ponudu"}</a>
        </div>
      </header>
      ...
      <main>
        ...
        <section id="cijene" className={`${frame} py-16 md:py-20`} aria-labelledby="pricing-title">
          <div className="grid gap-9 lg:grid-cols-[0.72fr_1.28fr] lg:gap-14"><div><p className={`${eyebrow} mb-3`}>Cijene</p><h2 id="pricing-title" className={heading}>Odaberite koliko pomoći trebate.</h2><p className="mt-5 leading-7 text-muted">Oba paketa su jednokratna. Prije potvrde provjeravamo osnovnu kompatibilnost i jasno navodimo svaku dodatnu prilagodbu.</p></div>
            <div className="grid gap-5 sm:grid-cols-2">
              <article className="flex flex-col rounded-2xl border border-line bg-white p-6"><span className="text-xs font-black uppercase tracking-widest text-teal-dark">Samo plugin</span><p className="mt-4 text-5xl font-black tracking-tight text-ink">{formatCjenikHrPrice(cjenikHrProduct.pluginPrice)}</p><p className="mt-2 text-sm font-bold text-muted">{isPreview ? "Jednokratno · TEST PLAĆANJE" : "Jednokratno · ponuda prije uplate"}</p><ul className="my-6 grid gap-3 text-sm leading-6 text-muted"><li>Plugin Cjenik HR {cjenikHrProduct.pluginVersion}</li><li>Dokumentirane funkcije proizvoda</li><li>Provjera osnovne kompatibilnosti prije isporuke</li></ul><a href="#upit" className="mt-auto inline-flex min-h-[50px] items-center justify-center rounded-lg border-2 border-teal px-5 text-sm font-black text-teal-dark hover:bg-teal-soft">{isPreview ? "Testiraj kupnju — 39 €" : "Zatražite ponudu"}</a></article>
              <article className="flex flex-col rounded-2xl border-2 border-teal bg-[#edf8f8] p-6 shadow-[0_22px_55px_rgba(0,77,88,0.12)]"><span className="text-xs font-black uppercase tracking-widest text-teal-dark">Plugin + postavljanje</span><p className="mt-4 text-5xl font-black tracking-tight text-ink">{formatCjenikHrPrice(cjenikHrProduct.setupPrice)}</p><p className="mt-2 text-sm font-bold text-muted">{isPreview ? "Jednokratno · TEST PLAĆANJE" : "Jednokratno · ponuda prije uplate"}</p><ul className="my-6 grid gap-3 text-sm leading-6 text-muted"><li>Sve iz paketa Plugin</li><li>Instalacija i početno podešavanje</li><li>Provjera osnovnog toka na vašem WordPressu</li></ul><a href="#upit" className={`${button} mt-auto`}>{isPreview ? "Testiraj kupnju s postavljanjem — 79 €" : "Zatražite ponudu"}</a></article>
            </div>
          </div>
        ...
        <section id="upit" className="border-y border-line bg-[#f5f9fa] py-16 md:py-20" aria-labelledby="request-title">
          <div className={`${frame} grid gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:gap-14`}><div><p className={`${eyebrow} mb-3`}>{isPreview ? "TEST PLAĆANJE" : "Ponuda bez obveze"}</p><h2 id="request-title" className={heading}>Provjerimo podatke prije uplate.</h2><p className="mt-5 leading-8 text-muted">{isPreview ? "Standardni paket vodi na sigurno testno plaćanje preko Stripea. Nema stvarne naplate, a račun i isporuka ostaju isključeni." : "Za standardni paket pripremamo ponudu s točnim iznosom i podatcima za uplatu."} Upiti za drugi sustav ostaju ručna procjena bez automatske naplate.</p><div className="mt-7 rounded-xl border border-line bg-white p-5"><p className="text-sm font-black text-ink">Ne trebamo pristupe za prvi korak.</p><p className="mt-2 text-sm leading-6 text-muted">Domena i opis sustava su dovoljni. Lozinke i poslovne datoteke razmjenjuju se samo naknadno, ako se dogovori siguran postupak.</p></div></div><CjenikHrRequestForm isPreview={isPreview} /></div>
        </section>
